...
GtkWidget *spelling;
GdkDisplay* Display;
static GtkWidget *recent_menu_item;

void tpad_ui_refresh_recent_menu(void)
{
	if (recent_menu_item != NULL)
		gtk_widget_set_sensitive(recent_menu_item,
		                         cfg_recent_files_enabled());
}

static void on_file_menu_show(GtkWidget *menu, gpointer user_data)
{
	(void) menu;
	(void) user_data;
...
static void on_recent_file_activate(GtkWidget *widget, gpointer user_data)
{
	const gchar *path;
	gchar *current;
	gchar *filename;
	...
	(void) user_data;
	path = g_object_get_data(G_OBJECT(widget), "tpad-recent-path");
	if (path == NULL)
		return;
...
static void on_recent_menu_show(GtkWidget *menu, gpointer user_data)
{
	GList *children;
	GPtrArray *files;
	GError *error = NULL;
	...
	if (!cfg_recent_files_enabled()) {
		GtkWidget *item = gtk_menu_item_new_with_label(
			gettext("Recent files are disabled"));
	...
	for (guint index = 0; index < files->len; index++) {
		...
		g_object_set_data_full(G_OBJECT(item), "tpad-recent-path",
		                       g_strdup(path), g_free);
		g_signal_connect(item, "activate",
		                 G_CALLBACK(on_recent_file_activate), NULL);
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
		gtk_widget_show(item);
	}
...
//tpad_fp_init();
	...
	GtkWidget *menubar,*file,*hashmenu,*hashmd5menu,*hhash, *hash256menu,*hash512menu,*ToBase64menu,*FromBase64menu,*hashfile512menu,*hashfile256menu,*fileToBase64menu,*fileFromBase64menu,*hashfilemd5menu,*filemenu,*recentmenu,*newfile,*open,*openRecent,*copyFileName, *save,*saveas,*print_item,*freqana,*quit,*checkOnOpen,
	...
	*scrolled_window,*mainbox,*lang_chk,*ShowFullPath,*lineWp,*keepAbove,*UIPrefs;
	...
	filemenu = gtk_menu_new();
		...
		openRecent = gtk_menu_item_new_with_mnemonic(
			gettext("Open _Recent"));
		recent_menu_item = openRecent;
		...
		gtk_menu_item_set_submenu(GTK_MENU_ITEM(openRecent), recentmenu);

	gtk_menu_shell_append(GTK_MENU_SHELL(filemenu), newfile);
	gtk_menu_shell_append(GTK_MENU_SHELL(filemenu), open);
	gtk_menu_shell_append(GTK_MENU_SHELL(filemenu), openRecent);
	...
	search = gtk_menu_item_new_with_label(_EDIT_MENU);
		...
		freqana=gtk_menu_item_new_with_label(gettext("Frequency Analysis"));
	...
	g_signal_connect(filemenu,"show",G_CALLBACK(on_file_menu_show),NULL);
	...
	g_signal_connect(recentmenu,"show",G_CALLBACK(on_recent_menu_show),NULL);
