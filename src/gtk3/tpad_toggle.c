...
void toggle_use_open_guard(GtkWidget *caller){
	...
	gboolean requested = gtk_check_menu_item_get_active(
		GTK_CHECK_MENU_ITEM(caller));
	gboolean enabled = tpad_open_guard_apply_enabled(requested, current);

	if (enabled != requested)
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(caller), enabled);
	cfg_set_use_open_guard((int) enabled);
	cfg_save();
	g_free(current);
}
////////////////////////////////////////////////////////////////////
